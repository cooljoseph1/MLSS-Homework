# -*- coding: utf-8 -*-
"""Adversarial Robustness.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vl35gwC-Ai2DDMPImMnViG3deOcfVLXd
...
def untargeted_FGSM(x_batch, true_labels, network, normalize, eps=8/255., **kwargs):
  ...
  x_input = normalize(x_batch)
  y_outputs = network(x_input)
  loss = loss_fn(y_outputs, true_labels)
  loss.backward()
  gradient = x_batch.grad
  difference = eps * gradient.sign()
  x_adv = x_batch + difference

  return x_adv

# Test the method
adversarial.test_untargeted_attack(untargeted_FGSM, eps=8/255.)
adversarial.test_untargeted_attack(untargeted_FGSM, eps=100/255.)
...
def targeted_FGSM(x_batch, target_labels, network, normalize, eps=8/255., **kwargs):
  ...
  x_input = normalize(x_batch)
  y_outputs = network(x_input)
  loss = loss_fn(y_outputs, target_labels)
  loss.backward()
  gradient = x_batch.grad
  difference = eps * gradient.sign()
  x_adv = x_batch - difference
...
def normalize_l2(x_batch):
    ...
    """
    flattened = x_batch.flatten(start_dim=-3)
    norm_squared = flattened.square().sum(dim=-1)
...
def tensor_clamp_l2(x_batch, center, radius):
    ...
    centered = x_batch - center
    flattened = centered.flatten(start_dim=-3)
    norm_squared = flattened.square().sum(dim=-1)
    norm = norm_squared.sqrt()
    norm[norm < radius] = radius
    centered *= radius
    centered /= norm

    clamped = centered + center
    return clamped

tensor_clamp_l2(torch.Tensor([
    [[[0.1]], [[0.1]], [[0.2]]]
]), 0, 1)
...
adversarial.test_targeted_attack(targeted_PGD, target_idx=10, eps=8/255.)
adversarial.test_targeted_attack(targeted_PGD, target_idx=100, eps=8/255.)
...
2. The adversarial trained model seems to be broken.  It always responds with a confidence of around $0.0033$ for "shovel".
3. Adding more step sizes makes both attacks more confident.
...
def adversarial_accuracy(x_batch, true_labels, network, normalize, num_steps=10, step_size=0.01, eps=8/255.):
    ...
    x_input = normalize(x_adv)
    y_outputs = network(x_input)
    predictions = torch.argmax(y_outputs, dim=-1)
    correct = predictions.eq(true_labels).sum()
...
num_steps_options = [1, 5, 10, 20, 50, 100]
accuracies = [adversarial_accuracy(x_batch, true_labels, network, normalization_function, num_steps=num_steps).item()
for num_steps in num_steps_options]

ax = plt.gca()
ax.plot(num_steps_options, accuracies)
