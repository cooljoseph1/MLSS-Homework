# -*- coding: utf-8 -*-
"""updated_two_layer_net.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1K5NzAtUC1oWii0_hEaxoYkRnlZ5p7vzY
...
def nn_loss_part2(params, X, y=None, reg=0.0):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    scores, hidden = nn_loss_part1(params, X, y, reg)

    # Softmax calculations #
    scores_probs = scores
    scores_max = scores_probs.max(-1, keepdim=True)
    scores_probs -= scores_max.values # numerical stability
    scores_probs = torch.exp(scores_probs)
    scores_probs /= scores_probs.sum(-1, keepdim=True)
    loss = -torch.log(scores_probs[torch.arange(N), y]).sum()

    loss /= N # We want an average loss, not total loss

    # Regularization
    loss += reg * W1.square().sum()
    loss += reg * W2.square().sum()
    ...
    # Calculate gradient for W2 and b2 using bias trick #
    hidden_extended = torch.cat((
        hidden,
        torch.ones_like(hidden[:, 0:1]), # Stack 1's for bias trick.
    ), dim=1)
    dW2_extended = hidden_extended.T @ scores_probs
    class_grad = torch.zeros_like(dW2_extended.T) # Gradient at correct class
    class_grad.index_add_(0, y, hidden_extended)
    dW2_extended -= class_grad.T

    dW2_extended /= N # Average over the entire batch
    dW2, dB2 = dW2_extended[:-1], dW2_extended[-1]
    dW2 += 2 * reg * W2 # gradient from regularization
    grads['W2'] = dW2
    grads['b2'] = dB2

    # Calculate gradient for W1 #
    h_grad = scores_probs @ W2.T - W2.T[y]
    h_grad *= hidden > 0
    X_extended = torch.cat((
        X,
        torch.ones_like(X[:, 0:1]), # Stack 1's for bias trick.
    ), dim=1)
    dW1_extended = X_extended.T @ h_grad
    dW1_extended /= N # Average over the entire batch
    
    dW1, dB1 = dW1_extended[:-1], dW1_extended[-1]
    dW1 += 2 * reg * W1 # gradient from regularization
    grads['W1'] = dW1
    grads['b1'] = dB1
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  class_preds = loss_func(params, X)
...
#################################################################################
# Replace "pass" statement with your code
reg, lr, hs = 0.001, 1.0, 128
best_net = TwoLayerNet(3 * 32 * 32, hs, 10, device=data_dict['X_train'].device)
stats = best_net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
          num_iters=3000, batch_size=1000,
          learning_rate=lr, learning_rate_decay=0.95,
          reg=reg, verbose=False)
stat_dict = {0: stats}
